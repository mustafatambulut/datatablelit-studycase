...
class DataTable extends LitElement {
  ...
  removeRow(row) {
    this.shadowRoot.querySelector("delete-confirm-modal").show(row);
  }
  ...
  render() {
    ...
    return html`
      ${this.selectedRows.length !== 0
        ? html`
            <div class="delete-selected" @click="${this.removeSelectedRows}">
              - ${translations.getTranslation("allDeleted")}
            </div>
          `
        : null}
      ${this.showModal
                  ...
                  ? Object.entries(this.editedRow || {})
                      ...
                      .map(([key, value]) =>
                        key !== "id"
                          ? html`
                              <label
                                >${translations.getTranslation(key)}:</label
                              >
                              <input
                                type="text"
                                .value="${value}"
      ...
      <delete-confirm-modal
        @delete-confirmed="${this.confirmDelete}"
      ></delete-confirm-modal>
    `;
  }
}
